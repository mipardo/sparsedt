...
class SparseTensorCOO:
    ...
    def from_dense_top_selection(cls, tensor, threshold):
        ...
        indexes = np.where(flattened_tensor.abs() >= threshold)[0]
    ...
    def slice(self, start, end, reset_indexes=False):
        ...
        """
        start_index = np.searchsorted(self.indexes, start, side='left')
        ending_index = np.searchsorted(self.indexes, end, side='left')

        sliced_values = self.values[start_index:ending_index]
        sliced_indexes = self.indexes[start_index:ending_index]
        if reset_indexes:
            sliced_indexes -= start
        
        return SparseTensorCOO(sliced_values, sliced_indexes, self.shape, self.has_canonical_format)
    ...
    def to_dense(self):
        ...
        dense_tensor = np.zeros(self.shape, dtype=np.float32)
        dense_tensor[np.unravel_index(self.indexes, self.shape)] = self.values
        return dense_tensor
    ...
    def __add__(self, other):
        ...
        summ_values = np.zeros(self.nnz + other.nnz)
        summ_indexes = np.zeros(self.nnz + other.nnz, dtype=np.int64)
